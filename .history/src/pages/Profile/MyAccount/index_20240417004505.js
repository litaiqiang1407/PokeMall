...
import { interactData, uploadFile } from "~/functions/interactData";
...
function MyAccount() {
  ...
  const handleAvatarChange = (e) => {
    ...
    if (file) {
      ...
      const selectedFileExt = file.name.split(".").pop().toLowerCase();

      if (allowedFileExt.includes(selectedFileExt)) {
        const formData = new FormData();
        formData.append("avatar", file);
        formData.append("id", userData.id);
        console.log("formData", formData);
        uploadFile(changeAccountInfoURL, "POST", formData, (response) => {
          if (response.message === "Avatar updated") {
            handleResponse("Avatar updated");
            setAvatarLink(response.avatar);
            setUserData((prevUserData) => ({
              ...prevUserData,
        ...
        handleResponse("Invalid file format");
      }
    }
  };
